...
<style>
   ...
   /* News item styling */
    ...
    /* Positive Highlight Styling */
    .positive-highlight {
        background-color: #b2f0b2; /* Light green for positive */
        padding: 2px 5px;
        border-radius: 3px;
        font-weight: bold;
    ...
    /* Negative Highlight Styling */
    .negative-highlight {
        background-color: #ffcccb; /* Light red for negative */
        padding: 2px 5px;
        border-radius: 3px;
        font-weight: bold;
...
# Helper function to generate analysis prompt - Modified to EXCLUDE references from LLM output and ask for positive/negative markers
...
1.  **Categorize Points:** Within each section of your analysis, identify key points as either **positive** or **negative** for the economic outlook of {country}. Some points might be neutral or mixed, but focus on clear positives and negatives where possible.
2.  **Mark Points:**
    *   Prefix each **positive** point with `[POSITIVE]: ` (e.g., `[POSITIVE]: The GDP growth is strong.`)
    *   Prefix each **negative** point with `[NEGATIVE]: ` (e.g., `[NEGATIVE]: Inflation rates are rising.`)
    *   For points that are neither clearly positive nor negative, or for general descriptive text, do not add any prefix.
                ...
                if full_response_text:
                    ...
                    for section in sections:
                        ...
                        def replace_references(match):
                            ...
                            superscript_refs = ', '.join([f"<sup>[{num}]</sup>" for num in ref_numbers]) # Create superscript refs
                        ...
                        for line in section_content.split('\n'):
                            if line.startswith("[POSITIVE]: "):
                                highlighted_line = f"<span class='positive-highlight'>{line[len('[POSITIVE]: '):]}</span>"
                            elif line.startswith("[NEGATIVE]: "):
                                highlighted_line = f"<span class='negative-highlight'>{line[len('[NEGATIVE]: '):]}</span>"
                            else:
                                highlighted_line = line
                            highlighted_content += highlighted_line + "\n"
                        section_content = highlighted_content
