...
st.set_page_config(
    page_title="Global Economic Analysis Hub",
    page_icon="🌍",
    layout="wide",
...
# Helper function to generate analysis prompt - Now uses news items with title, summary, full_text
def get_analysis_prompt(country, news_items):
    news_summary_list = []
    reference_list = []
    if news_items:
        for i, item in enumerate(news_items):
            news_summary_list.append(f"  <span class=\"reference\" onclick=\"showModal({i})\">{i + 1}</span> {item['summary']}")
            reference_list.append(f"#### Reference {i + 1}\n**Title:** {item['title']}\n\n**Summary:** {item['summary']}\n\n**Full Text:** {item['full_text']}\n\n**Source:** [{item['source']}]({item['url']})")
        news_summary = "<br>".join(news_summary_list)  # Use HTML line breaks
        references = "\n".join(reference_list)
    else:
...
Incorporate the following latest economic, bond market, stock market, and yield curve news headlines and summaries in your analysis (click on the numbers to view the full article):
<p>{news_summary}</p>

Provide a detailed analysis covering:

1. **Executive Summary:** Briefly summarize the current economic situation, key challenges, and outlook.
2. **Macroeconomic Indicators:** Analyze key macroeconomic indicators such as GDP growth, inflation, unemployment, interest rates, and exchange rates. Provide current figures and trends.
3. **Sector Analysis:** Examine the performance of key economic sectors (e.g., manufacturing, services, agriculture, technology). Identify strengths, weaknesses, and opportunities in each sector.
4. **Policy Environment:** Discuss relevant government policies and regulations impacting the economy, including fiscal policy, monetary policy, and trade policy.
5. **Financial Market Analysis:** Analyze the current conditions of the bond market, stock market, and yield curve, including recent trends and their potential impact on the economy.
6. **Risks and Opportunities:** Identify potential economic risks and challenges as well as opportunities for growth and development.
7. **Economic Outlook:** Provide a forward-looking perspective on the country's economic prospects for the next 1-3 years. Include forecasts and potential scenarios.

Format the report in markdown with clear headers and subheaders. Be concise and data-driven. Highlight key findings and important data points using bold or italic text.
"""
            ...
            try:
                ...
                if full_response_text:
                    ...
                    for section in sections:
                        ...
                        # Index news sources in section content
                        for index, news in news_source_mapping.items():
                            section_content = section_content.replace(f"({index})", f"<sup>[{index}]</sup>")
