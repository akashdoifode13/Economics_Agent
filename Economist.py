...
import io  # Import for creating in-memory file - not needed anymore
# from markdown2pdf import convert_from_markdown  # Import for PDF generation - not needed anymore
...
# Chart plotting functions from the first code block - START
...
def analyze_country_charts(country, data_path):
    ...
    st.subheader("Economic Indicator Charts") # Add a subheader for charts
    for indicator, params in datasets.items():
        ...
        if data is not None:
            ...
            st.warning(f"") # Inform user if chart data is missing
# Chart plotting functions from the first code block - END
...
# Enhanced CSS styling (rest of the CSS code is the same as provided in the prompt)
st.markdown("""
<style>
...
# Helper function to fetch article content using newspaper3k (rest of the functions are the same as provided in the prompt)
...
def fetch_economic_news(country, num_articles=10): # Adjusted default to 10
    ...
    try:
        ...
        search_queries = [
            ...
            f"{country}+economic+outlook", # Added economic outlook
            f"{country}+financial+stability", # Added financial stability
            f"{country}+economic+indicators", # Added economic indicators
            f"{country}+fiscal+policy",      # Added fiscal policy
            f"{country}+monetary+policy",    # Added monetary policy
            f"{country}+trade+balance",      # Added trade balance
            f"{country}+inflation+rate",     # Added inflation rate
            f"{country}+interest+rates",     # Added interest rates
            f"{country}+unemployment+rate",  # Added unemployment+rate
            f"{country}+consumer+confidence", # Added consumer confidence
            f"{country}+business+sentiment",  # Added business sentiment
            f"{country}+bond+market+news",
            f"{country}+stock+market+news",
            f"{country}+yield+curve+news",
            f"{country}+exchange+rate+news", # Added exchange rate news as requested
            f"{country}+economic+growth+news", # Added economic growth news
            f"{country}+sovereign+debt+news",  # Added sovereign debt news
            f"{country}+credit+rating+news", # Added credit rating news
            f"{country}+banking+sector+news"  # Added banking sector news
        ]
        headers = {
...
def get_analysis_prompt(country, news_items):
    ...
    if news_items:
        for i, item in enumerate(news_items):
            ...
            reference_list.append(f"*{item['title']}*. {item['source']}, {item['time']}. URL: {item['url']}")
...
def display_economic_news(country):
    ...
    # Display news with loading animation - Spinner moved to the analysis tab for step-by-step process
...
if country_name:
    if not api_key:
        ...
        with tabs[0]: # Economic Analysis Tab
            ...
            status_placeholder.info("Fetching latest economic news...", icon="🔍") # Initial message
            ...
            if news_for_analysis:
                ...
                status_placeholder.warning("Could not fetch economic news. Analysis will proceed without news context.", icon="⚠️")

            status_placeholder.info("Generating economic analysis using Gemini...", icon="🤖") # Update message
            ...
            try:
                ...
                status_placeholder.success("Analysis generated!", icon="✅") # Update message

                status_placeholder.info("Processing and displaying analysis...", icon="✍️") # Update message
                # Process and display the response with news source indexing
                if full_response_text:
                    sections = full_response_text.split("## ")
                    formatted_response = ""
                    news_source_mapping = {i+1: news for i, news in enumerate(news_for_analysis) if news} # Ensure news is not None before enumerate
                    ...
                    for section in sections:
                        ...
                        # Index news sources in section content - Handle multiple references
                        def replace_references(match):
                            ref_numbers_str = match.group(0)[1:-1] # Extract numbers string inside brackets
                            ref_numbers = [num.strip() for num in ref_numbers_str.split(',')] # Split and strip numbers
                            superscript_refs = []
                            for num in ref_numbers:
                                try:
                                    ref_index = int(num)
                                    if ref_index in news_source_mapping:
                                        article_url = news_source_mapping[ref_index]['url'] # Get URL from mapping
                                        superscript_refs.append(f"<a href='{article_url}' target='_blank' style='text-decoration:none;'><sup>[{num}]</sup></a>") # Add <a> tag with URL
                                    else:
                                        superscript_refs.append(f"<sup>[{num}]</sup>") # Fallback if index not found (should not happen)
                                except ValueError:
                                    superscript_refs.append(f"<sup>[{num}]</sup>") # Handle non-integer references if any
                    ...
                    if news_source_mapping: # Only add references if there are news items
                        formatted_response += "\n### References\n"
                        formatted_response += "<ol>\n" # Start ordered list for references
                        for index, news in news_source_mapping.items():
                            formatted_response += f"  <li> {news['title']}. <i>{news['source']}, {news['time']}</i>. <a href='{news['url']}'>[Link to Article]</a></li>\n" # More detailed and formatted reference
                        formatted_response += "</ol>\n" # Close ordered list

                    # More aggressive stripping: Remove trailing whitespace and then </div> tags
                    formatted_response = formatted_response.rstrip() # Remove trailing whitespace first
                    formatted_response = formatted_response.rstrip('</div> </div>') # Then remove divs again

                    # Even more aggressive stripping (remove any trailing closing div tags):
                    while formatted_response.endswith('</div>'): # Keep stripping until no more trailing divs
                        formatted_response = formatted_response[:-6].rstrip() # Remove '</div>' and trailing whitespace
                    ...
                    """, unsafe_allow_html=True)
                    status_placeholder.success("Analysis displayed!", icon="✅") # Final update

                    # Display charts after text analysis
                     # Optional data path input
                    analyze_country_charts(country_name, data_path_input)


            except Exception as e:
                status_placeholder.error(f"Analysis generation failed: {str(e)}", icon="❌") # Update to error in case of failure
